...
const [messageCounter,setMessageCounter]=useState('')
  ...
  const { selectedConversation,addMessage, setSelectedConversation, messages } =
    userConversation();
  const { onlineUser, socket } = useContext(SocketContext);

  const nowOnline = chatUser.map((user) => {
    user._id;
  });
...
useEffect(() => {
  ...
  return (
    <>
      ...
      <div
        className={`flex flex-col w-full md:w-80 bg-gray-800 border-r border-gray-700 ${
              ...
              {searchUser.map((user) => (
                ...
                >
                  <div className="relative inline-block">
                    ...
                    {isOnline && (
                      <span className="absolute bottom-9 left-9 w-2.5 h-2.5 bg-green-400 border-green-50  rounded-full"></span>
                    )}
                  </div>
                  ...
                  {chatUser.map((user) => (
                    ...
                    >
                      <div className="relative inline-block">
                        ...
                        {/* Green dot when online */}
                        {isOnline && (
                          <span className="absolute bottom-9 left-9 w-2.5 h-2.5 bg-green-400 border  rounded-full"></span>
                        )}
                      </div>
                      ...
                      {
                        messageCounter.receiverId===authUser._id && messageCounter.senderId===user._id ?(<><div class="relative inline-block">
                        <span class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-green-400 font-bold text-[10px]  text-black shadow-md">
                          +1
                        </span>
                      </div></>):(<></>)
                      }
